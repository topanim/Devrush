...
open class HubCommunicationTask : DefaultTask() {
    ...
    fun generate() {
        ...
        FileSpec.builder("eu.lepicekmichal.signalrkore", "HubCommunication")
            ...
            .addImport(
                packageName = "io.eduonline.devrush.devrushlmsmultiplatform.libs.signalr",
                "OnValue1",
                "OnValue1",
                "OnValue2",
                "OnValue3",
                ...
                TypeSpec.classBuilder("HubCommunication")
                    ...
                    .addFunction(
                        FunSpec.builder("send")
                            ...
                            .addParameter(
                                name = "args",
                                type = List::class.asTypeName()
                                    .parameterizedBy(JsonElement::class.asTypeName())
                            )
                            .addParameter(
                    ...
                    .addFunction(
                        FunSpec.builder("handleIncomingInvocation")
                            ...
                            .receiver(
                                Flow::class.asTypeName()
                                    .parameterizedBy(TypeVariableName(name = "HubMessage.Invocation"))
                            )
                                ...
                                type = LambdaTypeName
                                    .get(
                                        parameters = listOf(
                                            ParameterSpec(
                                                "value",
                                                TypeVariableName(name = "HubMessage.Invocation")
                                            )
                                        ),
                                        returnType = genericTypeVariableT,
                                    )
            ...
            argsAndStreamsCombination(reifiedParameters = reified) { argumentTypes, streamTypes ->
                ...
                addOutFunction(
                    ...
                    modifiers = listOfNotNull(
                        KModifier.SUSPEND,
                        if (reified) KModifier.INLINE else null
                    ),
                    hasReifiedTypes = reified,
                    argumentTypes = argumentTypes,
                    streamTypes = streamTypes,
    ...
    private fun TypeSpec.Builder.addOnWithResults(reified: Boolean): TypeSpec.Builder {
        return this.apply {
            val resultTypeVariable = TypeVariableName(
                name = "RESULT",
                bounds = listOf(Any::class)
            ...
            paramsCombination(reifiedParameters = reified) { paramTypes ->
                addInFunction2(
                    ...
                    extraParameters = listOfNotNull(
                        if (!reified) ParameterSpec(
                            "resultType",
                            resultTypeVariable.inKClass
                        ) else null,
                        ParameterSpec(
                            name = "callback",
        ...
        return addFunction(
            FunSpec.builder(name = name)
                .apply {
                    if (name != jvmName) {
                        addAnnotation(
                            AnnotationSpec.builder(JvmName::class).addMember("\"$jvmName\"").build()
                        )
                    }
                }
