...
import eu.lepicekmichal.signalrkore.HubCommunication
...
abstract class HubCommunicationLink(private val json: Json) : HubCommunication() {
    ...
    private val invocationIdLock = SynchronizedObject()

    @OptIn(InternalAPI::class)
    private fun getInvocationIdAndIncrement() = synchronized(invocationIdLock) {
        return@synchronized invocationId++
    }
    ...
    final override fun <T : Any> T.toJson(kClass: KClass<T>): JsonElement =
        json.encodeToJsonElement(kClass.serializer(), this)

    @OptIn(InternalSerializationApi::class)
    final override fun <T : Any> JsonElement.fromJson(kClass: KClass<T>): T =
        json.decodeFromJsonElement(kClass.serializer(), this)
    ...
    final override fun send(
        ...
        val streamIds = streams.map { uuid4().toString() }
    ...
    final override suspend fun invoke(
        ...
        processResult = {
            logger.log(
                ...
                "Result of a completion message has been ignored: ${it.result}",
                null
            )
        },
    )
        ...
        processResult = {
            try {
                ...
                throw RuntimeException(
                    "Completion result could not be parsed as ${resultType.simpleName}: ${it.result}",
                    ex
                )
            } catch (ex: IllegalArgumentException) {
                throw RuntimeException(
                    "${resultType.simpleName} could not be initialized from the completion result: ${it.result}",
                    ex
                )
            }
        },
    ...
    private suspend fun <T : Any> internalInvoke(
        ...
        val invocationId = getInvocationIdAndIncrement().toString()

        val streamIds = uploadStreams.map { uuid4().toString() }
        ...
        val completion = receivedCompletions.filter { it.invocationId == invocationId }.first()
        co.touchlab.kermit.Logger.d("d") { "completion: $completion" }
    ...
    final override fun <T : Any> stream(
        ...
        val invocationId = uuid4().toString()
        ...
        return callbackFlow {
                ...
                receivedStreamItems
                    ...
                    .map {
                        try {
                            ...
                            throw RuntimeException(
                                "Completion result could not be parsed as ${itemType.simpleName}: ${it.item}",
                                ex
                            )
                        } catch (ex: IllegalArgumentException) {
                            throw RuntimeException(
                                "${itemType.simpleName} could not be initialized from the completion result: ${it.item}",
                                ex
                            )
                        }
                    }
            ...
            launch {
                when (val completion =
                    receivedCompletions.filter { it.invocationId == invocationId }.first()) {
                    is HubMessage.Completion.Error -> throw RuntimeException(completion.error)
