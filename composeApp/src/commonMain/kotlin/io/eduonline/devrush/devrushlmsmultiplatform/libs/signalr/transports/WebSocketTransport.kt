...
import io.eduonline.devrush.devrushlmsmultiplatform.libs.signalr.HubMessage
import io.eduonline.devrush.devrushlmsmultiplatform.libs.signalr.Transport
import io.eduonline.devrush.devrushlmsmultiplatform.libs.signalr.utils.headers
...
internal class WebSocketTransport(
    ...
    override suspend fun start(url: String) {
        ...
        session = client.webSocketSession(urlString = formattedUrl) {
            ...
            timeout {
                requestTimeoutMillis = 60_000 //HttpTimeout.
            }
        }
    ...
    override fun receive(): Flow<ByteArray> = session?.incoming
        ...
        ?.map { it.readBytes() }
        ?.catch {
            if (it is EOFException) emit((Json.encodeToString(HubMessage.Close()) + io.eduonline.devrush.devrushlmsmultiplatform.libs.signalr.RECORD_SEPARATOR).toByteArray())
            else throw it
        }
        ?: throw IllegalStateException("WebSocket connection has not been started")
